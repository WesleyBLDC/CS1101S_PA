...
function is_dna_strand(xs) {
    ...
    // if(is_nucleobase(head(xs))===false){
    //     return false;
    //     break;
    // }else{
...
function combine(xss) {
    ...
    if (is_null(xss)){
        return null;
    }else if (tail(xss)===null){
        return head(xss);//forgot to head(xss);
    }
    else    
    {
...
function oxoguanine_repair(xs) {
    ...
    // WRITE HERE.
    return map(x => x==="8" ? x="G" : x=x, xs); //its a string!!!!
...
//
    // WRITE HERE.
    //find first occurence of string "A" "T" "G"
    return is_null(xs)
        ? null
        : tail(xs) === null
            ? null
            : (head(xs)==="A" && head(tail(xs))==="T" && head(tail(tail(xs))) ==="G") //whats my mistake here, what can i do?
                ? tail(tail(tail(xs)))
