...
function all_different(nums) {
    ...
    //input random values, return true IF all are unique values.
...
function is_valid_toto_set(nums, n, min, max) {
    ...
    const max_value = accumulate((elem,x)=>math_max(elem,x),max2,nums);
    // display(max_value);
    
    const min_value = accumulate((elem,x)=>math_min(elem,x),min2,nums);
    // display(min_value);
    
    //must also check if unique 
    
    return (nums === len && min_value === min && max_value === max)
        ? true
        : false;
